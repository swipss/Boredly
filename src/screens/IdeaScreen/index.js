    ...
    try {
      const response = await fetch(
        `http://www.boredapi.com/api/activity?${
          ...
          accessibility && `accessibility=${accessibility.toFixed(2)}`
    ...
    <View style={styles.container}>
      ...
      {activity.activity ? (
        <>
          ...
          <Text style={styles.info}>
            {activity.type} &#8226; {activity.participants} participants &#8226;
            ${activity.price} &#8226;{" "}
            {activity.accessibility < 0.5
              ? "accessible"
              : "possibly unaccessible"}
          </Text>
        </>
      ) : (
      ...
      {activity.activity && (
        <Pressable onPress={() => onLike()}>
          <AntDesign
            name={checkIfInActivities() ? "heart" : "hearto"}
            size={36}
            color={checkIfInActivities() ? "#ff0050" : "black"}
          />
        </Pressable>
      )}
