...
export default function SavedScreen() {
  ...
  const removeActivity = async (key) => {
    ...
    try {
      ...
      const activitiesItems = activitiesFav.filter(function (e) {
        return e.key !== key;
      });
  ...
  const storeCompletedActivity = async (activity) => {
    try {
      let existingCompletedActivities =
        await getExistinCompletedActivitiesFromStorage();
      const updatedCompletedActivities = [
        ...existingCompletedActivities,
        activity,
      ];
      console.log("completed activities", updatedCompletedActivities);
      await AsyncStorage.setItem(
        "completed",
        JSON.stringify(updatedCompletedActivities)
      );
      ...
      <SwipeListView
        ...
        renderHiddenItem={({ item }, rowMap) => (
          <View style={styles.rowBack}>
            <Pressable
              onPress={() => removeActivity(item.key)}
              style={styles.delete}
            >
